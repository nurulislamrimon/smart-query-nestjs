import qs from 'qs';
import { Injectable, createParamDecorator, Global, Module } from '@nestjs/common';
...
function parseFieldFilter(key, value) {
  const rangeMatch = key.match(/^(.+)\[(gte|gt|lte|lt)\]$/);
  if (rangeMatch) {
    return {
      field: rangeMatch[1],
      operator: rangeMatch[2],
      value
    };
  }
...
function buildDateFilter(operator, value) {
...
function buildStringFilter(operator, value) {
  switch (operator) {
    case "eq":
